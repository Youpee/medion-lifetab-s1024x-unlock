...
> **Which branch this applies to:** the app suite, native-lib baking, debloat and the English default
> (§1–4, §9) ship on **all** branches. **Magisk, microG + signature spoofing, and the root
> notification-shade firewall (§2.6, §5, §7, §8) are `root`-branch only** — they need root and are not
> present on `main`/`docker`.
