...
|---|---|---|
| **[`main`](https://github.com/Youpee/medion-lifetab-s1024x-unlock)** | **Arch / Linux**, no root | Kiosk removed + launcher + **a full open-source app suite** (browser, camera, gallery, files, keyboard, F-Droid, Aurora Store). Native, simplest. |
| **`docker`** (you are here) | **Windows / macOS / any Linux**, no root | **Exactly the same result + the same app suite**, but the offline build runs in a **container**, so it works on any OS. |
| **[`root`](https://github.com/Youpee/medion-lifetab-s1024x-unlock/tree/root)** | want a **fully usable system** | Everything above **plus ROOT (Magisk) and microG** (with automatic signature spoofing), a dark-theme default and the shade helper — and the fixes for what the stripped Medion build leaves broken. |
...
tablet is usable out of the box with **no Google and no trackers**. The container build fetches them
