...
- **Windows / macOS (or any distro without the build tools)** — the scripts on `main` are
  Linux/Arch-first. A **cross-platform, universal build via Docker** (the offline build runs
  inside a container, so it works on Windows/macOS/Linux) lives in the **`docker` branch** —
  use that branch if you're not on Arch:
  https://github.com/Youpee/medion-lifetab-s1024x-unlock/tree/docker
  (USB steps still need native mtkclient — on Windows: WSL2 + `usbipd-win`, or run it natively.)
