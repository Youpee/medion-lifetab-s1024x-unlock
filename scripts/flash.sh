...
if [ -n "$BOOT" ]; then
  ...
  printf '%s  ##########################################################%s\n' "$Y" "$N"
  printf '%s  #  FIRST SETUP self-settles over a couple of reboots:      #%s\n' "$Y$B" "$N"
  printf '%s  #  the tablet REBOOTS ITSELF ONCE to enable microG spoofing #%s\n' "$Y$B" "$N"
  printf '%s  #  (Zygisk/LSPosed). Just let it. Give it ~5 min to settle. #%s\n' "$Y$B" "$N"
  printf '%s  ##########################################################%s\n' "$Y" "$N"
